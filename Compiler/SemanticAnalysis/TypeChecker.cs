    ...
    public class TypeChecker
    {
        ...
        private void PerformTypeChecking(IAbstractSyntaxTreeNode node)
        {
            if (node is null)
            {
                ...
                Reporter.AddError(node.Position, "Tried to perform type checking on a null tree node");
            }
            else if (node is ErrorNode)
            {
                ...
                Reporter.AddError(node.Position, "Tried to perform type checking on an error tree node");
            }
            else
            {
                ...
                if (function == null)
                {
                    ...
                    Reporter.AddError(node.Position, $"Couldn't find the function {functionName} when type checking");
                }
                else
                {
        ...
        private void PerformTypeCheckingOnAssignCommand(AssignCommandNode assignCommand)
        {
            ...
            if (!(assignCommand.Identifier.Declaration is IEntityDeclarationNode varDeclaration))
            {
                // Error - identifier is not a variable
                Reporter.AddError(assignCommand.Position, "Identifier is not a Variable");
            }
            else
            {
                if(varDeclaration.EntityType != assignCommand.Expression.Type)
                {
                    // Error - expression is wrong type for the variable
                    Reporter.AddError(assignCommand.Position, "Expression is wrong type for the variable");
                }
        ...
        private void PerformTypeCheckingOnCallCommand(CallCommandNode callCommand)
        {
            ...
            if (!(callCommand.Identifier.Declaration is FunctionDeclarationNode functionDeclaration))
            {
                ...
                Reporter.AddError(callCommand.Position, "Identifier is not a function");
            ...
            else if (GetNumberOfArguments(functionDeclaration.Type) == 0)
            {
                if (!(callCommand.Parameter is BlankParameterNode))
                {
                    ...
                    Reporter.AddError(callCommand.Position, "function takes no arguments but is called with one");
                }
            }
            else
            {
                if (callCommand.Parameter is BlankParameterNode)
                {
                    ...
                    Reporter.AddError(callCommand.Position, "function takes an argument but is called without one");
                }
                else
                {
                    if (GetArgumentType(functionDeclaration.Type, 0) != callCommand.Parameter.Type)
                    {
                        ...
                        Reporter.AddError(callCommand.Position, "Function called with parameter of the wrong type");
                    }
                    if (ArgumentPassedByReference(functionDeclaration.Type, 0) && !(callCommand.Parameter is VarParameterNode))
                    {
                        Reporter.AddError(callCommand.Position, "Function requires a var parameter and also call paramater is not var");
                    }
                    if (ArgumentPassedByReference(functionDeclaration.Type, 0))
                    {
                        if (!(callCommand.Parameter is VarParameterNode))
                        {
                            ...
                            Reporter.AddError(callCommand.Position, "Function requires a var parameter but has been given an expression parameter");
                    ...
                    else
                    {
                        if (!(callCommand.Parameter is ExpressionParameterNode))
                        {
                            ...
                            Reporter.AddError(callCommand.Position, "Function requires an expression parameter but has been given a var parameter");
        ...
        private void PerformTypeCheckingOnIfCommand(IfCommandNode ifCommand)
        {
            ...
            if (ifCommand.Expression.Type != StandardEnvironment.BooleanType)
            {
                ...
                Reporter.AddError(ifCommand.Position, "expression needs to be a boolean");
            }
        }
        ...
        /// Carries out type checking on an if command node
        /// </summary>
        /// <param name="ifElseCommand">The node to perform type checking on</param>
        private void PerformTypeCheckingOnIfElseCommand(IfElseCommandNode ifElseCommand)
        {
            ...
            {
                // Error: expression needs to be a boolean
                Reporter.AddError(ifElseCommand.Position, "expression needs to be a boolean");
            }
        }
        ...
        private void PerformTypeCheckingOnWhileCommand(WhileCommandNode whileCommand)
        {
            ...
            {
                // Error: expression needs to be a boolean
                Reporter.AddError(whileCommand.Position, "expression needs to be a boolean");
        ...
        private void PerformTypeCheckingOnRepeatCommand(RepeatCommandNode repeatCommand)
        {
            ...
            PerformTypeChecking(repeatCommand.UntilExpression);
            if (repeatCommand.UntilExpression.Type != StandardEnvironment.BooleanType)
            {
                // Error: expression needs to be a boolean
                Reporter.AddError(repeatCommand.Position, "expression needs to be a boolean");
        ...
        private void PerformTypeCheckingOnBinaryExpression(BinaryExpressionNode binaryExpression)
        {
            ...
            if (!(binaryExpression.Op.Declaration is BinaryOperationDeclarationNode opDeclaration))
            {
                ...
                Reporter.AddError(binaryExpression.Position, "operator is not a binary operator");
            ...
            else
            {
                if (GetArgumentType(opDeclaration.Type, 0) == StandardEnvironment.AnyType)
                {
                    if (binaryExpression.LeftExpression.Type != binaryExpression.RightExpression.Type)
                    {
                        ...
                        Reporter.AddError(binaryExpression.Position, "left and right hand side arguments not the same type");
                    }
                }
                else
                {
                    if (GetArgumentType(opDeclaration.Type, 0) != binaryExpression.LeftExpression.Type)
                    {
                        ...
                        Reporter.AddError(binaryExpression.Position, "Left hand expression is wrong type");

                    }
                    if (GetArgumentType(opDeclaration.Type, 1) != binaryExpression.RightExpression.Type)
                    {
                        // Error: Right hand expression is wrong type
                        Reporter.AddError(binaryExpression.Position, "Right hand expression is wrong type");
        ...
        private void PerformTypeCheckingOnIdExpression(IdExpressionNode idExpression)
        {
            ...
            if (!(idExpression.Identifier.Declaration is IEntityDeclarationNode declaration))
            {
                ...
                Reporter.AddError(idExpression.Position, "identifier is not a variable or constant");
            }
            else
        ...
        private void PerformTypeCheckingOnUnaryExpression(UnaryExpressionNode unaryExpression)
        {
            ...
            if (!(unaryExpression.Op.Declaration is UnaryOperationDeclarationNode opDeclaration))
            {
                ...
                Reporter.AddError(unaryExpression.Position, "operator is not a unary operator");
            ...
            else
            {
                if (GetArgumentType(opDeclaration.Type, 0) != unaryExpression.Expression.Type)
                {
                    ...
                    Reporter.AddError(unaryExpression.Position, "expression is the wrong type");
        ...
        private void PerformTypeCheckingOnVarParameter(VarParameterNode varParameter)
        {
            ...
            {
                // Error: identifier is not a variable
                Reporter.AddError(varParameter.Position, "identifier is not a variable");
        ...
        private void PerformTypeCheckingOnTypeDenoter(TypeDenoterNode typeDenoter)
        {
            ...
            if (!(typeDenoter.Identifier.Declaration is SimpleTypeDeclarationNode declaration))
            {
                ...
                Reporter.AddError(typeDenoter.Position, "identifier is not a type");
        ...
        private void PerformTypeCheckingOnCharacterLiteral(CharacterLiteralNode characterLiteral)
        {
            ...
            {
                // Error - value too big   
                Reporter.AddError(characterLiteral.Position, "value too big");
            }
        }
        ...
        private void PerformTypeCheckingOnIntegerLiteral(IntegerLiteralNode integerLiteral)
        {
            ...
            {
                // Error - value too big
                Reporter.AddError(integerLiteral.Position, "value too big");
