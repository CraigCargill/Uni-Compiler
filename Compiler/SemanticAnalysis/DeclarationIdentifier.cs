    ...
    public class DeclarationIdentifier
    {
        ...
        private void PerformIdentification(IAbstractSyntaxTreeNode node)
        {
            if (node is null)
            {
                ...
                Reporter.AddError(node.Position, "Tried to perform identification on a null tree node");
            }
            else if (node is ErrorNode)
            {
                ...
                Reporter.AddError(node.Position, "Tried to perform identification on an error tree node");
            }
            else
            {
                ...
                if (function == null)
                {
                    ...
                    Reporter.AddError(node.Position, $"Couldn't find the function {functionName} when doing identification");
                }
                else
        ...
        private void PerformIdentificationOnRepeatCommand(RepeatCommandNode repeatCommand)
        {
            ...
            PerformIdentification(repeatCommand.UntilExpression);
        }
