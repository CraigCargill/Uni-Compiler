...
namespace Compiler.SyntacticAnalysis
{
    ...
    public class Parser
    {
        ...
        private void Accept(TokenType expectedType)
        {
            ...
            {
                Reporter.AddError(CurrentToken.Position, $"Accepted Token is not of correct type. ({CurrentToken.Type}) found when expecting ({expectedType})");
            }
        }
        ...
        private ICommandNode ParseCommand()
        {
            ...
            while (isCommandBeginningTypes(CurrentToken.Type))
            {
                commands.Add(ParseSingleCommand());
                Accept(Semicolon);
            }
        ...
        private ICommandNode ParseAssignmentOrCallCommand()
        {
            ...
            {
                Reporter.AddError(CurrentToken.Position, $"TokenType {CurrentToken.Type} found when expecting LeftBracket or Becomes");
        ...
        private ICommandNode ParseIfCommand()
        {
            ...
            switch (CurrentToken.Type)
            {
                ...
                case (NoElse):
                    Accept(NoElse);
                    noElseCommandNode noElseCommand = new noElseCommandNode(startPosition);
                    return new IfCommandNode(expression, thenCommand, noElseCommand, startPosition);
                default:
                    Reporter.AddError(CurrentToken.Position, $"TokenType {CurrentToken.Type} found when expecting else or noelse");
                    return new ErrorNode(startPosition);
            }
        ...
        private ICommandNode ParseRepeatCommand()
        {
            ...
            Accept(Until);
            IExpressionNode untilExpression = ParseExpression();

            return new RepeatCommandNode(command, untilExpression, startPosition);
        }
        ...
        private IDeclarationNode ParseDeclaration()
        {
            ...
            Accept(Semicolon);
            while (isDeclerationBeginningTypes(CurrentToken.Type))
            {
                declarations.Add(ParseSingleDeclaration());
                Accept(Semicolon);
            }
        ...
        private IDeclarationNode ParseSingleDeclaration()
        {
            ...
            {
                Debugger.Write("Parsing Type Denoter Declaration");
            ...
            {
                Reporter.AddError(CurrentToken.Position, "Trying to parse decleration that is not valid");
        ...
        private IExpressionNode ParsePrimaryExpression()
        {
            ...
            switch (CurrentToken.Type)
            {
                ...
                default:
                    Reporter.AddError(CurrentToken.Position, "Trying to parse illegal primary expression");
                    return new ErrorNode(CurrentToken.Position);
            }
        }
        ...
        private IExpressionNode ParseIdExpression()
        {
            ...
            {
                Accept(LeftBracket);
                IParameterNode parameterNode = ParseParameter();
                Accept(RightBracket);
                return new CallExpressionNode(identifier, parameterNode);
            }
            else
            {
        ...
        /// <summary>
        /// Checks if type is starting command type
        /// </summary>
        /// <returns>A bool of whether or not this command is a starting command type</returns>
        private bool isCommandBeginningTypes(TokenType typeToCheck)
        {
            Debugger.Write("Checking types against list of begin types");
        ...
        /// <summary>
        /// Checks if type is starting decleration type
        /// </summary>
        /// <returns>A bool of whether or not this command is a starting decleration type</returns>
        private bool isDeclerationBeginningTypes(TokenType typeToCheck)
        {
            Debugger.Write("Checking types against list of decleration types");
            List<TokenType> types = new List<TokenType>();
            types.Add(Identifier);
            types.Add(Const);
